...
9. **Pre-existing abliterated model on the dev workstation** — ~~open,
   remove vs. document~~ **Resolved 2026-09-12: removed.** Phase 4.1
   found Ollama already running on `cyber` with a third-party,
   safety-training-stripped ("abliterated") 20B model already pulled
   (`hf.co/huihui-ai/Huihui-gpt-oss-20b-BF16-abliterated:Q4_K_M`, 15GB),
   predating and unrelated to this project's use of a small 1-3B
   placeholder model (`llama3.2:1b`). User decision: remove rather than
